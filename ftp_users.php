<?php
/**
 * Gerenciamento de Contas FTP (Pure-FTPd)
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        $dir = trim($_POST['dir'] ?? '');
        $uid = intval($_POST['uid'] ?? 2000);
        $gid = intval($_POST['gid'] ?? 2000);
        ...
        if (empty($username) || empty($password) || empty($dir)) {
            ...
            try {
                ...
                if ($stmt->fetchColumn() > 0) {
                    ...
                    $stmt->execute([$username, $encrypted_pass, $dir, $uid, $gid, $ul_bandwidth, $dl_bandwidth, $status, $device_id]);
                    
                    // Cria automaticamente a pasta local se o mapeamento estiver ativo
    ...
    if ($action === 'edit') {
        ...
        if (empty($old_username) || empty($username) || empty($dir)) {
            ...
            try {
                // Se a senha foi preenchida, atualiza senha também
                if (!empty($password)) {
                    $encrypted_pass = encrypt_ftp_password($password);
                    $stmt = $pdo->prepare("UPDATE `ftp_users` SET `User` = ?, `Password` = ?, `Dir` = ?, `Uid` = ?, `Gid` = ?, `ULBandwidth` = ?, `DLBandwidth` = ?, `Status` = ?, `device_id` = ? WHERE `User` = ?");
                    $stmt->execute([$username, $encrypted_pass, $dir, $uid, $gid, $ul_bandwidth, $dl_bandwidth, $status, $device_id, $old_username]);
                } else {
                    $stmt = $pdo->prepare("UPDATE `ftp_users` SET `User` = ?, `Dir` = ?, `Uid` = ?, `Gid` = ?, `ULBandwidth` = ?, `DLBandwidth` = ?, `Status` = ?, `device_id` = ? WHERE `User` = ?");
                    $stmt->execute([$username, $dir, $uid, $gid, $ul_bandwidth, $dl_bandwidth, $status, $device_id, $old_username]);
                }
                
                // Cria a nova pasta local correspondente se necessário
                $local_path = get_real_ftp_path($dir);
                if (!file_exists($local_path)) {
                    mkdir($local_path, 0777, true);
                }
...
if (isset($_GET['delete'])) {
    ...
    try {
        ...
        if ($stmt->fetchColumn() > 0) {
            $stmt = $pdo->prepare("DELETE FROM `ftp_users` WHERE `User` = ?");
            $stmt->execute([$user_to_delete]);
            $success_msg = "Usuário FTP '$user_to_delete' excluído com sucesso do banco de dados!";
        }
    } catch (PDOException $e) {
...
// Buscar usuários FTP com detalhes dos dispositivos associados
try {
    $ftp_users = $pdo->query("SELECT f.*, d.name as device_name FROM ftp_users f LEFT JOIN devices d ON f.device_id = d.id ORDER BY f.User ASC")->fetchAll();
            ...
            <tbody>
                <?php if (count($ftp_users) > 0): ?>
                    <?php foreach ($ftp_users as $user): ?>
                        <tr>
                            <td style="font-weight: 600; color: white; font-family: monospace;">
                            ...
                            <td style="font-size: 0.85rem; color: var(--text-secondary);">
                                <?php 
                                    $ul = $user['ULBandwidth'] > 0 ? $user['ULBandwidth'] . ' KB/s' : 'Ilimitado';
                                    $dl = $user['DLBandwidth'] > 0 ? $user['DLBandwidth'] . ' KB/s' : 'Ilimitado';
                                    echo "▲ $ul / ▼ $dl";
                                ?>
                            </td>
                            <td>
                ...
                <?php else: ?>
                    <tr class="no-results">
                        <td colspan="7" class="text-center empty-state" style="text-align: center;">
                            ...
                            <p style="font-size: 0.85rem; margin-top: 0.25rem;">Clique no botão superior para criar sua primeira conta FTP.</p>
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
        ...
        <form action="" method="POST">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="password" id="add-password" name="password" class="form-control" placeholder="Digite uma senha forte" required autocomplete="new-password">
                </div>
            </div>
            
            <div class="form-group">
                <label for="add-dir">Diretório Home *</label>
                <input type="text" id="add-dir" name="dir" class="form-control" placeholder="Ex: /home/ftpusers/router_core" required>
                <p style="font-size: 0.75rem; color: var(--text-muted); margin-top: 0.25rem;">
                    * As pastas locais serão criadas dinamicamente no servidor web para permitir a gestão dos backups.
                </p>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    <label for="add-uid">UID (Linux User ID)</label>
                    <input type="number" id="add-uid" name="uid" class="form-control" value="2000" required>
                </div>
                <div class="form-group">
                    <label for="add-gid">GID (Linux Group ID)</label>
                    <input type="number" id="add-gid" name="gid" class="form-control" value="2000" required>
                </div>
            </div>
            
            <div class="form-row">
                <div class="form-group">
                    <label for="add-ul-bandwidth">Upload Máx (KB/s)</label>
                    <input type="number" id="add-ul-bandwidth" name="ul_bandwidth" class="form-control" value="0" placeholder="0 = ilimitado">
                </div>
                <div class="form-group">
                    <label for="add-dl-bandwidth">Download Máx (KB/s)</label>
                    <input type="number" id="add-dl-bandwidth" name="dl_bandwidth" class="form-control" value="0" placeholder="0 = ilimitado">
                </div>
            </div>
    ...
    <div class="modal-content">
        ...
        <form action="" method="POST">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    <label for="edit-ftp-password">Senha FTP (Deixe em branco para manter a atual)</label>
            ...
            <div class="form-row">
                <div class="form-group">
                    <label for="edit-ftp-uid">UID (Linux User ID)</label>
                    <input type="number" id="edit-ftp-uid" name="uid" class="form-control" required>
                </div>
                <div class="form-group">
                    <label for="edit-ftp-gid">GID (Linux Group ID)</label>
            ...
            <div class="form-row">
                <div class="form-group">
                    <label for="edit-ftp-ul-bandwidth">Upload Máx (KB/s)</label>
                    <input type="number" id="edit-ftp-ul-bandwidth" name="ul_bandwidth" class="form-control">
                </div>
                <div class="form-group">
                    <label for="edit-ftp-dl-bandwidth">Download Máx (KB/s)</label>
...
<!-- Script Adicional para a UX de auto-preenchimento do diretório home -->
<script>
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (addUsernameInput && addDirInput) {
        addUsernameInput.addEventListener('input', function() {
            ...
            // Auto preenche o diretório home padrão Linux
